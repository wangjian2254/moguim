...
def setWeiNotePoint(weiNotePoint):
    memcache.set('WeiNotePoint' + str(weiNotePoint.key().name()), weiNotePoint, 360000)


def getWeiNotePoint(id):
    """

    :param id: 帖子id
    :return: 返回帖子积分对象
    """
    weiNotePoint = memcache.get('WeiNotePoint' + str(id))
    if not weiNotePoint:
        weiNotePoint = WeiNotePoint.get_by_key_name('n' + str(id))
